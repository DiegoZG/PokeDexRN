  ...
  if (!pokemon) {
    return (
      <View className="flex-1 justify-center items-center bg-gray-50">
        <Text className="text-xl text-gray-600">Pokemon not found</Text>
      </View>
    );
  }
        ...
        >
          <View className="items-center mt-4">
            ...
            <View className="bg-white w-full rounded-t-3xl p-6">
              <View className="mb-8">
                <Text className="text-2xl font-bold text-gray-800 mb-4">Base Stats</Text>
                {pokemon.stats?.map((stat, index) => (
                  <View key={index} className="mb-4">
                    <View className="flex-row justify-between mb-2">
                      <Text className="capitalize text-gray-600 font-medium">
                        {stat.name.replace('-', ' ')}
                      </Text>
              ...
              <View className="mb-8">
                ...
                <View className="flex-row flex-wrap justify-between">
                  <View className="bg-gray-50 rounded-2xl p-4 mb-4 w-[48%]">
                    <Text className="text-gray-500 mb-1">Height</Text>
                    <Text className="text-xl font-bold text-gray-800">{(pokemon.height / 10).toFixed(1)}m</Text>
                  </View>
                  <View className="bg-gray-50 rounded-2xl p-4 mb-4 w-[48%]">
                    <Text className="text-gray-500 mb-1">Weight</Text>
                    <Text className="text-xl font-bold text-gray-800">{(pokemon.weight / 10).toFixed(1)}kg</Text>
                  </View>
                  <View className="bg-gray-50 rounded-2xl p-4 w-full">
                    <Text className="text-gray-500 mb-1">Base Experience</Text>
                    <Text className="text-xl font-bold text-gray-800">{pokemon.base_experience}</Text>
                  </View>
                </View>
              </View>

              <View>
                <Text className="text-2xl font-bold text-gray-800 mb-4">Abilities</Text>
                <View className="flex-row flex-wrap">
                  {pokemon.abilities?.map((ability, index) => (
                    <View
                      key={index}
                      className="bg-gray-50 rounded-full px-4 py-2 mr-2 mb-2"
                    >
